...
(() => {
  ...
  // LAYOUT_EDITOR_START
  // LAYOUT_EDITOR:initializer_START
  ...
  // TAILOR_ENGINE_START
  // TAILOR_ENGINE:formatter_START

      return String(path).split('/').map(encodeURIComponent).join('/');

  // TAILOR_ENGINE:formatter_END
  // TAILOR_ENGINE_END
    }
...
async function ghRequest(token, url, options = {}) {
  ...
  // TAILOR_ENGINE:parser_START
      ...
      const name = String(funcName).replace(/[.*+?^${}()|[\]\\]/g, '\\function initTailor(ctx) {
    if (window.__TAILOR_INITIALIZED__) {
      console.log('Tailor already initialized, skipping...');
      return;
    }
