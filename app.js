...
function initUploadSystem() {
  ...
  function updateCategoryPreview() {
    ...
    if (currentCategory) {
      ...
      preview.textContent = 'Enter a category name';
  ...
  function updateFilePreview(index) {
    ...
    if (sanitized && currentCategory) {
      preview.textContent = `Path: ${currentCategory}/${sanitized}${extension}`;
      preview.style.color = '#4CAF50';
    } else if (!currentCategory) {
      preview.textContent = 'Select a category first';
      preview.style.color = '#f44336';
    } else {
      preview.textContent = 'Enter a filename';
      preview.style.color = '#f44336';
    }
  }
  ...
  closeUploadBtn?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      uploadUI?.classList.add('upload-hidden');
      resetUploadUI();
    }, 300);
  });
  ...
  fileInput?.addEventListener('change', (e) => {
    ...
    if (files.length > 0) {
      ...
      uploadBtn?.classList.remove('upload-hidden');
    } else {
      fileList.innerHTML = '';
      uploadBtn?.classList.add('upload-hidden');
    }
  });
  ...
  uploadBtn?.addEventListener('click', async () => {
    ...
    uploadProgress.classList.remove('upload-hidden');
    uploadResults.classList.add('upload-hidden');
  ...
  function resetUploadUI() {
    ...
    fileList.innerHTML = '';
    uploadBtn?.classList.add('upload-hidden');
    uploadProgress?.classList.add('upload-hidden');
    uploadResults?.classList.add('upload-hidden');
    uploadResults.innerHTML = '';
